...
def draw_bboxes_for_events(events, ax, boolean_mask=None, linewidth=2):
    ...
    for event_i in range(len(events)):
        ...
        event_length = events[event_i].event_length
        ...
        xy, width, height = (t_start_orig, 0), event_length, 2000 - 30
        ...
        line_handler = ax.plot(x_values, y_values, linewidth=linewidth)
        c = line_handler[0].get_color()  # 'r'

        # Draw a rectangle around event:
        # xy, width, height
        rect = Rectangle(xy, width, height, linewidth=linewidth, edgecolor=c, facecolor='none')
        ax.add_patch(rect)
...
def extract_events_data_from_dataset(logger, coarse_timestamp, strong_coherence_threshold = 5.9, weak_coherence_threshold = 3.0, event_length=4.0, merge = 44):
    ...
    non_ev = extract.CoherentEventSegmenter(coherence_threshold=weak_coherence_threshold,merge=merge,event_length=event_length, metric_comparison_function_over=smaller_than_coherence)
    ...
    # events might not need this filtering ...
    maximal_SS_for_nonevent = 0.003
    final_non_events = [nonoverlapping_non_events[i] for i in range(len(nonoverlapping_non_events)) if
                        (nonoverlapping_non_events[i].SS < maximal_SS_for_nonevent and nonoverlapping_non_events[i].SS > 0.0)]
...
def visualize_events_nonevents(logger, coarse_timestamp, events, non_events, show=False, save=None, close=True, linewidth = 2):
    ...
    ax_list = fig.axes
    draw_bboxes_for_events(events, ax_list[0], linewidth=linewidth)
    draw_bboxes_for_events(non_events, ax_list[1], linewidth=linewidth)
...
if __name__ == '__main__':
    ...
    #strong_coherence_threshold = 5.9 #<3rd run
    #weak_coherence_threshold = 2.5
    #event_length = 2.0
    #merge = 10

    main_folder = "samples-s"+str(strong_coherence_threshold)+"w"+str(weak_coherence_threshold)+"_"+str(event_length)+"sec"+"_"+str(merge)+"merge"
    plots_directory = main_folder+"/" # plots/
    mkdir(plots_directory)
    #mkdir("samples/")
    events_directory = main_folder+"/events/"
    mkdir(events_directory)
    nonevents_directory = main_folder+"/nonevents/"
    ...
    for selected_logger in loggerlist:
        for selected_coarse_timestamp in coarse_timestamps:
            ...
            events, non_events = extract_events_data_from_dataset(selected_logger, selected_coarse_timestamp, strong_coherence_threshold, weak_coherence_threshold, event_length, merge)
            visualize_events_nonevents(selected_logger, selected_coarse_timestamp, events, non_events, show=False, save=plots_directory, linewidth = linewidth)
